...
class MongoDBManage:
    ...
    def create_stock_index(self):
        #建立索引標籤
        self.Taiwan_stock.create_index([("stockID", ASCENDING)])
...
if __name__ == "__main__":
    ...
    mongodb.Taiwan_stock.create_index([("stockID", ASCENDING)])
    is_exist = mongodb.Taiwan_stock.find({"stockID":todayinfo[1]})
    for stockItem in is_exist:
        if stockItem["stockID"] == todayinfo[1]:
